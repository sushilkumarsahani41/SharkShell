...
Open **Settings → MCP Access** and click **New Access Key**. For each key you choose:
...
Supported methods: `initialize`, `ping`, `tools/list`, `tools/call`. Notifications (no `id`, e.g. `notifications/initialized`) return `202` with no body. Auth failures return HTTP `401` with a JSON-RPC error (`code: -32001`).
